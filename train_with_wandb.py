...
from training_functions_with_wandb import trains
...
def main():
    # WandB logging
    wandb.init(project='DARP-original', entity='dongbin_na')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        print('pseudo_orig\'s distribution:', torch.sum(pseudo_orig, dim=0))
        print('argmax(pseudo_orig)\'s class distribution (hard label in FixMatch):', torch.bincount(torch.argmax(pseudo_orig, dim=1), minlength=10))
        print('targets_u\'s distribution (same as target distribution):', torch.sum(pseudo_refine, dim=0))
        print('argmax(targets_u)\'s class distribution (hard label in FixMatch):', torch.bincount(torch.argmax(pseudo_refine, dim=1), minlength=10))
